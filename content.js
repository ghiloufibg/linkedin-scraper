window.runLinkedinSearchAndScrape = async function (keywords, maxPosts) {
  ...
  const LABELS = ["past month", "dernier mois", "le mois dernier"];
  ...
  const searchBox = await waitFor("input.search-global-typeahead__input", {
    visible: true,
  });
  searchBox.focus();
  ...
  const dateFilterBtn =
    document.querySelector("#searchFilter_datePosted") ||
    document.querySelector('[aria-label*="Date de publication"]') ||
    document.querySelector('[aria-label*="Date posted"]');

  if (!dateFilterBtn)
    throw new Error("Bouton 'Date de publication' introuvable");
  dateFilterBtn.click();
  await delay(400);

  const pastMonthLabel =
    document.querySelector('label[for="datePosted-past-month"]') ||
    [...document.querySelectorAll("label")].find((l) => {
      const txt = normalize(l.innerText);
      return LABELS.some((w) => txt.includes(normalize(w)));
    });

  if (!pastMonthLabel)
    throw new Error("Label 'Dernier mois / Past month' introuvable");
  ...
  const applyBtn =
    pastMonthLabel
      .closest("form")
      ?.querySelector(
        'button[aria-label*="Apply"], button[aria-label*="Appliquer"]',
      ) ||
  ...
  if (!applyBtn) throw new Error("Bouton 'Afficher les résultats' introuvable");
  applyBtn.click();
  ...
  function extractPost(post) {
    let auteur = cleanText(
      post.querySelector(
        ".update-components-actor__title span[class='visually-hidden']",
      )?.innerText,
    );
    ...
    let jobUrl =
      post
        .querySelector(".update-components-entity__content-wrapper a")
        ?.getAttribute("href") || "N/A";
    let salaireMatch = description.match(
      /(\d{1,3}(?:[\s.,]\d{3})*|\d+)(?:\s?(€|euros?|k|K|m|M))/,
    );
